...
@section('content_header')
    <h1 class="m-0 text-dark">Hormonas</h1>
@stop
                    ...
                    <td>{{ $h->created_at }}</td>
                        <td width="15px">
                            <div class="d-flex">
                                ...
                                <a href="{{route('hormona.edit', $h->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="EDITAR">
                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                </a>
                                <a href="{{route('hormona.show', $h->id) }}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="EDITAR">
                                    <i class="fa fa-lg fa-fw fas fa-eye"></i>
                                </a>
                                <a href="{{route('hormona.edit', $h->id) }}" class="btn btn-xs btn-default text-danger mx-1 shadow" title="PDF">
                        ...
                        <x-adminlte-modal id="modalCustom{{ $h->id }}" title="Eliminar" size="sm" theme="warning" icon="fa-solid fa-triangle-exclamation" v-centered static-backdrop scrollable>
                            ...
                            <x-slot name="footerSlot">
                                <form action="{{ route('hormona.destroy', $h->id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
